...
> **State correction, 2026-09-22. `master` is `80a45b1`.** PR **#25** merged
> on the 21st at 20:45 UTC and that is what production runs. Read this note
> before the 2026-09-20 one below, which says `af46f6d` and was true until
> then.
>
> **Three PRs opened on the 21st are green and waiting on you.** All three are
> based on `80a45b1` with `master` merged into them, both checks green on
> that merge, none merged:
>
> - **#28** `claude/counter-dedupe` — one shared fixed-window counter, one
>   Redis client where a process used to build two. No behaviour change.
> - **#29** `claude/tap-targets` — every header control 48x48.
> - **#30** `claude/retention-cleanup` — a daily sweep of expired sign-in
>   codes and `authSession` rows. **The only one of the three that changes
>   production behaviour**, and it needs **`CRON_SECRET`** set on Vercel
>   (Production only) before or with the merge. It also leaves a /privacy
>   wording decision to you. Both are in its body and in Open items.
>
> **The three older PRs are still based on `af46f6d`, and their greens
> predate the #25 merge**: **#24** (question-media), **#26** (/how-it-works,
> copy still awaiting your approval), **#27** (print page-break). Nothing is
> known to be wrong with any of them — but their `test` runs never saw the
> current `master`, and #24 and #26 have not been re-tested against the
> header #25 rewrote. Merge `master` into each and let CI re-run before
> trusting those greens. See the Open items entry on stale greens.
>
> **#31** (`claude/pricing-45` — annual at $45, and the "no account" copy that
> accounts made wrong) appeared on the 21st and is untouched by this session.
> **#4** (Paddle) is untouched and still your call.
...
*Updated 2026-09-22. The three entries immediately below are new. Two older
entries are now marked **ADDRESSED** — each has an open PR against it and
stays here until that PR merges. Everything else is the 2026-09-19 list.*

- **`CRON_SECRET` is waiting on the owner, and PR #30 does nothing without
  it.** Name only: **`CRON_SECRET`**, **Production scope only** (Vercel does
  not run crons on previews), marked sensitive, value generated by you. No
  value is written in this repo, in any PR body, or here, and none should be.
  Merged *without* it nothing breaks — the route refuses everyone including
  the scheduler, so the cron 401s daily and deletes nothing. That fail-closed
  behaviour was confirmed by running the route with the variable unset, not
  inferred. Order of operations and what the first run's log should say are
  in PR #30's body.

- **/privacy's retention wording is your call, and the code is about to get
  ahead of it.** Once #30 is merged *and* its first run is visible in
  Vercel's cron log, the `#retention` clause can say an expired code or
  session is deleted by a daily clean-up, instead of only that it stops
  working. The suggested sentence is in #30's body. Either wording needs a
  `LEGAL_LAST_UPDATED` bump in the same commit. **Do not change the wording
  before a real run has been seen** — the policy must not get ahead of the
  code, which is the whole reason it was written the weaker way.

- **A PR's green check only means what its base was.** #28, #29 and #30 were
  all green on `test` and `Vercel` — and then `master` moved under them when
  #25 merged, which rewrote `AccountNav.tsx`, the very file #29 changes. Those
  greens were re-earned by merging `master` in and re-running the whole gate
  and CI. **#24, #26 and #27 have not had that done.** When a PR has been open
  across a merge to `master`, its check is stale evidence, and it is stale
  silently — GitHub still shows a green tick. This is the cheapest mistake to
  make in this repo and among the most expensive to find later.
...
- **ADDRESSED by PR #30, not merged. Nothing sweeps an expired sign-in code
  nobody used, or an expired `authSession` row.** A code is deleted
  atomically when it *is* used (`consumeVerificationValue` deletes before it
  checks expiry, so even a stale one cleans itself up). Both stop working on
  time; neither is removed. This is why the retention clause promises only
  that they stop working.
  - **Correction found while building #30:** expired `verification` rows
    *are* cleared, but only opportunistically. Better Auth 1.7.5 deletes
    **every** expired verification row whenever it next looks one up
    (`findVerificationValue`; `verification.disableCleanup` is not set) —
    i.e. whenever anyone submits a code or returns from Google. On a busy day
    that is minutes; on a quiet week it is never. So "nothing sweeps them"
    was half right. Expired `authSession` rows have no such path and were
    never removed at all. One of #30's tests pins Better Auth's behaviour so
    an upgrade that drops it becomes visible.
...
- **ADDRESSED by PR #29, not merged. The header's tap targets are 36px.**
  "Sign in" and "Sign out" are 68x36 and 69x36, under the 44px iOS / 48px
  Material minimum. They match the four existing nav links exactly, so this is
  the header's house style rather than something accounts regressed — but it
  now applies to the two controls a host taps most on a phone. Changing it
  means restyling the existing links too, which is why it was left alone.
  - **The account controls were not the worst of it.** Measuring every control
    in a real browser, on `master`: all 60 (both rows x 320/390/1280 x signed
    in/out) are 36px tall, and on the **homepage** the links carry no
    horizontal padding at all, so each one's pressable area is exactly its
    text box. **"Join" is 24px wide** — half the minimum in *both* dimensions,
    on the one control a stranger taps on their own phone to get into a game.
    Create is 42, Packs 37, Pricing 44. On `/pricing` (SiteHeader) the links
    already had `px-3` and only the height was ever wrong. That asymmetry is
    why the fix restyles both rows.
...
New in the 2026-09-21 session — all three are sandbox mechanics, none affect
CI or production, and each cost real time before it was understood:

- **Do not symlink `node_modules` into a git worktree.** Turbopack refuses it
  outright — `FATAL: An unexpected Turbopack error occurred` /
  `Symlink [project]/node_modules is invalid, it points out of the filesystem
  root` — and it takes both `next build` and `next dev` down, so every e2e
  test fails in milliseconds and it looks like the change broke the app. Copy
  the directory instead (`cp -a`, ~1.1GB per worktree) or run `npm install` in
  each worktree.
- **The sandbox's Chromium is not the build Playwright pins.**
  `@playwright/test` 1.62.1 wants chromium 1234; the image ships **1194** at
  `/opt/pw-browsers`, and `cdn.playwright.dev` is blocked by the network
  policy, so `npx playwright install` dies with
  `403 ... request blocked: no rule or allowlist entry allows host
  "cdn.playwright.dev"`. Run the suite against the browser that is here, via a
  throwaway config setting
  `launchOptions.executablePath = "/opt/pw-browsers/chromium"`, and delete
  that config before `git add`. **Do not change `playwright.config.ts`** — CI
  downloads its own matching browser and the committed config is correct
  there.
- **`pkill -f` and `pgrep -f` will match the shell you are typing in.** A
  pattern like `next dev -p 4601` appears in your own command line, so the
  kill takes out the session itself (exit 144) and leaves the server running.
  Kill by walking `/proc/*/cmdline`, or by port.
...
## What landed in the 2026-09-21 session — the counter, the sweep, the targets

Three PRs, one per job, all against `master` `80a45b1`, all green on both
`test` and `Vercel`, none merged. Each was re-gated *after* `master` moved,
and each was then driven in the running app rather than trusted to its suite.

### PR #28 — `claude/counter-dedupe`, one counter and one Redis client

`src/lib/rate-limit.ts` and `src/lib/daily-ceiling.ts` each carried their own
Upstash client, their own fallback `Map` and their own INCR/EXPIRE body, so a
process that loaded both built **two Redis clients against one instance** and
every fix to the mechanism had to be made twice. `src/lib/fixed-window-counter.ts`
is now the one client and the one counter: `hit`, `secondsLeft`, `release`
(DECR, with the never-negative repair SET carrying its expiry explicitly —
moved over from daily-ceiling unchanged). Each caller makes its own counter,
so each keeps its own fallback Map and daily-ceiling's `__resetMemoryCounters`
still clears only its own keys.

Behaviour is identical at both call sites for every limit any caller passes.
One internal difference, stated so "identical" stays honest: the rate
limiter's *in-memory* fallback used to stop counting once a bucket was full,
where Redis kept counting; both now keep counting, as Redis always did. That
changes no answer at any limit of 1 or more. At a limit of **0** the old
fallback let the first request of a window through where Redis refused it;
both now refuse. **No caller passes 0** — re-checked: routes pass 5, 20, 30,
40, 150; `ANSWER_SUBMISSIONS_PER_QUESTION` is 5; Better Auth's `customRules`
are 5, 10, 10.

Driven live, not just tested: `POST /api/packs/generate` six times from one
IP gives five 401s (the limiter passing, auth refusing) then **429 with
`Retry-After: 600`**; a second IP immediately after still gets 401, so
buckets are still keyed per IP. `POST /api/auth/email-otp/send-verification-otp`
six times gives five 200s then 429 — that is the *other* caller,
`consumeRateLimit`, on the same counter. The release path was exercised by
setting `FREE_DAILY_PACK_CEILING=1` with no `ANTHROPIC_API_KEY`: three
generate calls in a row all returned `503 notConfigured` and never the
ceiling refusal, which can only happen if the reserved unit was handed back
each time (`MissingApiKeyError` is one of the two cases where the ceiling
*is* refunded — route.ts:179-183). That the ceiling refusal is reachable at
all was confirmed separately at `FREE_DAILY_PACK_CEILING=0`, which returns
`{"dailyCeilingReached":true,"limit":0}`.

### PR #30 — `claude/retention-cleanup`, the daily sweep

`GET /api/cron/retention` deletes every `verification` and `authSession` row
whose `expiresAt` has passed, and nothing else. `vercel.json` (a new file,
setting only `crons`) runs it daily at 03:00 UTC. The guard
(`src/lib/cron-auth.ts`) accepts only `Authorization: Bearer <CRON_SECRET>`,
compares timing-safely as `admin-auth.ts` does, and **fails closed**.
No schema change and no migration.

Driven live against a database populated through the real sign-in endpoints:
no header / wrong secret / lower-case `bearer` all give 401; the real header
gives 200. With both rows still live the sweep reports `0, 0` and leaves them.
Aged an hour into the past, it reports **`{"verifications":1,"sessions":1}`**,
the rows are gone and **the `user` row survives**. The server logged
`retention sweep: 1 expired codes, 1 expired sessions` — the line Vercel's
cron log will show. With `CRON_SECRET` unset, every request 401s **including
the previously-valid secret**, and a seeded expired row survived two calls.
`POST` returns 405 (Next's method handling, before the guard).

### PR #29 — `claude/tap-targets`, 48px everywhere in the header

Every control in both copies of the row — SiteHeader and the homepage's own —
is now at least 48x48, via `min-h-12 min-w-12` with centred content. The extra
height reaches into the space around the row (`-my-1.5`) rather than pushing
it open, so where the row is one line nothing moves.

Measured in a real browser on both branches: **60 controls, 60 under 48px on
`master`, 0 under 48px on the branch.** Header heights came out exactly as the
PR body claims — `/pricing` **66px at 1280 on both** (unchanged), 94 -> 96 at
560, 130 -> 144 at 320-430. Zero layout shift at all five widths signed in and
out, with the session request held open so the pending state is really
observed, and no horizontal overflow on any of five pages at 320/360/390/430.
The worst target turned out not to be an account control — see Open items.

### The thing worth remembering from this session

All three PRs were already written and already green when the session started.
The work that mattered was noticing that **`master` had moved underneath them**
— #25 merged at 20:45 UTC, after their CI ran at 19:55 — and that #25 rewrote
the one file #29 touches. A green tick on a PR does not go stale visibly. See
the Open items entry.

## Next

*Rewritten 2026-09-22 against `master` `80a45b1`. The previous list was the
2026-09-19 one and had gone stale in both its lead items — #21, #22 and #23
all merged on the 20th — and in item 3, whose branch is superseded by PR #27.*

0. **Set `CRON_SECRET` on Vercel, then decide #30.** Production scope only,
   sensitive, a value you generate — it is never written down here. This is
   the one thing standing between #30 and a working sweep, and #30 is the
   only one of the three new PRs that changes production behaviour. The order
   of operations, and what the first run's log should show, are in its body.
   Merged without the variable, nothing breaks: the cron 401s and deletes
   nothing.

1. **Decide #28, #29 and #30.** All three are green on `test` and `Vercel`
   against the current `master`, and merge cleanly with each other in any
   order. #28 needs nothing from you but the merge. #29 would benefit from a
   look on a real phone first — `/pricing` and `/` at phone width, signed in
   and out — since every measurement behind it is an emulated viewport.

2. **Bring #24, #26 and #27 up to the current `master` before trusting their
   greens.** All three are still based on `af46f6d` and their checks ran
   before #25 merged; #24 and #26 have never been tested against the header
   #25 rewrote. Merge `master` into each and let CI re-run. **#26's copy is
   still awaiting your approval** and that has not changed.

3. **Confirm Upstash is configured on production.** Unchanged, and still the
   most important item on this list. Without
   `UPSTASH_REDIS_REST_URL`/`_TOKEN` the daily generation ceiling falls back
   to an in-process counter per serverless instance, so "20 a day" is really
   20 a day *per instance* and the Anthropic bill is not capped the way the
   code claims. **#28 does not change this either way** — it makes the
   fallback shared between the two counters, not correct in production.

4. **Verify the generation fixes against the real model on production** — the
   three-step gate and its cautions are in Open items. Still the oldest unpaid
   debt here, and a different thing from the decline verification, which was
   done on the 18th.

5. **Task 10, the live Paddle cutover** (owner-gated), then phase 3b (Tasks
   11-12). Its Website-approval prerequisite has been satisfied since PR #6.
   Note PR #4 still cannot ship as written once accounts are live — the
   `customData.creatorId` problem in Open items — and #31 changes the annual
   price it quotes.
